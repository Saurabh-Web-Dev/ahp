  ...
  <div class="d-flex">
    ...
    <div class="container-fluid p-4 main-content">
      ...
      <div class="card shadow-sm p-4 text-center">
        <h5 class="text-primary mb-3">🩺 Manage Today's Queue</h5>
        <router-link to="/doctor/appointment/list" class="btn btn-success px-4">
          View Queue
        </router-link>
      </div>
    </div>
...
const fetchDashboardData = async () => {
  try {
    const res = await axios.get('/api/doctor/dashboard-stats')
...
onMounted(async () => {
  ...
  if (!auth.user || auth.user.role !== 'doctor') router.push('/')
